...
public sealed class PostgresAiV2RecommendationTests
{
    ...
    public async Task SessionResultsFeedbackAndRetention_WorkOnMigratedPostgres()
    {
        ...
        try
        {
            ...
            var now = FixtureUtcNow;
            var allCandidates = await new FoodRecommendationReadRepository(db).GetCandidatesAsync(now, 500, CancellationToken.None);
            var candidate = Assert.Single(allCandidates, value => value.FoodId == FoodId);
            Assert.Equal(FoodId, candidate.FoodId);
            Assert.Equal(80_000, candidate.CurrentPrice);
            Assert.Equal("ING_TEST_BEEF", Assert.Single(candidate.IngredientCodes));
    ...
    private static readonly DateTime FixtureUtcNow = new(2026, 8, 4, 12, 0, 0, DateTimeKind.Utc);
    ...
    private static async Task SeedParents(string connectionString)
    {
        ...
        await using var command = new NpgsqlCommand(
            ...
            ('77777777-7777-7777-7777-777777777101','TEST_BEEF','ING_TEST_BEEF','Thịt bò test',true,true,0,now(),now());
