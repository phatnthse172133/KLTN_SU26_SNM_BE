...
public sealed class CustomerFoodProfileService(IFoodSemanticMetadataRepository repository) : ICustomerFoodProfileService
{
    ...
    public async Task<ApiResponse<CustomerFoodProfileResponse>> UpdateMineAsync(Guid customerId, UpdateCustomerFoodProfileRequest request, CancellationToken ct = default)
    {
        ...
        if (ingredients.Count != request.PreferredIngredientIds.Concat(request.AvoidedIngredientIds).Distinct().Count()
            ...
            throw AppException.BadRequest("One or more food profile catalog IDs are invalid or inactive.", "FOOD_PROFILE_INVALID_REQUEST");
    ...
    private static void Validate(UpdateCustomerFoodProfileRequest request)
    {
        ...
        { if (values.Count != values.Distinct().Count()) throw AppException.BadRequest($"{label} must not contain duplicates.", "FOOD_PROFILE_INVALID_REQUEST"); }
        ...
        if (request.PreferredIngredientIds.Intersect(request.AvoidedIngredientIds).Any()) throw AppException.BadRequest("An avoided ingredient cannot also be preferred.", "FOOD_PROFILE_INGREDIENT_CONFLICT");
        if (request.PreferredTasteProfileIds.Intersect(request.AvoidedTasteProfileIds).Any()) throw AppException.BadRequest("An avoided taste cannot also be preferred.", "FOOD_PROFILE_TASTE_CONFLICT");
        if (request.PreferredPriceMin > request.PreferredPriceMax) throw AppException.BadRequest("Preferred minimum price cannot exceed maximum price.", "FOOD_PROFILE_INVALID_REQUEST");
        if (request.PreferredCourses.Any(x => !Enum.IsDefined(x))) throw AppException.BadRequest("Preferred course is invalid.", "FOOD_PROFILE_INVALID_REQUEST");
        if (request.PreferredSpiceLevel.HasValue && !Enum.IsDefined(request.PreferredSpiceLevel.Value)) throw AppException.BadRequest("Preferred spice level is invalid.", "FOOD_PROFILE_INVALID_REQUEST");
        if (request.DefaultMaxDistanceMeters <= 0) throw AppException.BadRequest("Default maximum distance must be greater than zero.", "FOOD_PROFILE_INVALID_REQUEST");
    }
